...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const generateRoutes = async (): Promise<RouteRecordRaw[]> => {
    ...
    const { data } = res;
    const scopedData = filterModuleScope(data);
    const sdata = JSON.parse(JSON.stringify(scopedData));
    const rdata = JSON.parse(JSON.stringify(scopedData));
    const defaultData = JSON.parse(JSON.stringify(scopedData));
  ...
  const filterModuleScope = (routeList: RouteRecordRaw[]): RouteRecordRaw[] => {
    // 仅保留当前项目需要展示的模块路由，避免无关菜单污染侧边栏。
    const allowPrefixes = ['/system', '/tenant', '/kaisi'];
    const moduleKeywords = ['租户', 'tenant', '开思', 'kaisi'];
    ...
    const pick = (list: RouteRecordRaw[], inherited = false): RouteRecordRaw[] => {
      const result: RouteRecordRaw[] = [];
      list.forEach((route) => {
        const currentPath = normalizePath(route.path as string);
        const routeTitle = (route.meta as any)?.title?.toString?.() || '';
        const routeName = route.name?.toString?.() || '';
        const byPath = allowPrefixes.some((prefix) => currentPath === prefix || currentPath.startsWith(`${prefix}/`));
        const byKeyword = moduleKeywords.some((keyword) => routeTitle.includes(keyword) || routeName.toLowerCase().includes(keyword));
        const selfAllowed = inherited || byPath || byKeyword;
        const cloned = JSON.parse(JSON.stringify(route)) as RouteRecordRaw;
        if (cloned.children && cloned.children.length > 0) {
          cloned.children = pick(cloned.children, selfAllowed);
        }
        if (selfAllowed || (cloned.children && cloned.children.length > 0)) {
          result.push(cloned);
        }
      });
      return result;
