...
from platforms.robot.crawler import RobotCrawler, normalize_supplier_filters, normalize_sku
...
def execute(task_id: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    quality_origin_ids = str(params.get("qualityOriginIds") or "").strip()
    ...
    single_sku_max_crawl_count = int(params.get("singleSkuMaxCrawlCount") or 0)

    browser = None
    try:
        browser = Browser(channel="msedge", headless=True, image=False)
        auth_manager = RobotAuthManager()
        ...
        crawler = RobotCrawler(
            ...
            single_sku_max_crawl_count=single_sku_max_crawl_count,
        )
        ...
        if not crawler.prepare_shared_query(shared_skus):
            raise RuntimeError("获取共享查询上下文失败，任务已停止")
        ...
        for index, product in enumerate(products, start=1):
            ...
            try:
                sku_records = crawler.crawl_sku(
                    sku=sku,
                    quality_origin_ids=quality_origin_ids,
                    supplier_ids=supplier_id,
                    brand_names=brand_names,
                    region_origin_ids=str(params.get("regionOriginIds") or ""),
                )
        ...
        report_log(task_id, f"[robot] 抓取完成，耗时: {round(time.time() - start_ts, 2)}s")
    finally:
        if browser is not None:
            try:
                browser.stop()
                report_log(task_id, "[robot] 浏览器已关闭")
            except Exception as exc:
                report_log(task_id, f"[robot] 关闭浏览器失败: {exc}", level="WARNING")
